...
interface MovableFABActionListener {
    fun onAction()
}
...
class MovableFAB : FloatingActionButton, OnTouchListener {
    ...
    var movableFABActionListener: MovableFABActionListener? = null
...
//                .alpha(0.5f)
            ...
            movableFABActionListener?.onAction()
    ...
    companion object {
        private const val CLICK_DRAG_TOLERANCE =
            1000f // Often, there will be a slight, unintentional, drag when the user taps the FAB, so we need to account for this.
    }
}